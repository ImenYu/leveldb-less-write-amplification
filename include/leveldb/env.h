...
class LEVELDB_EXPORT Env {
 public:
  ...
  // opens an existing object with the specified filename. 
  // On success, stores a pointer of the file in
  // *result and returns OK.
  // On failure(i.e. the file does not exist), stores a nullptr in *result and returns non-OK
  virtual Status OpenFileAsRandomReWrFile(const std::string &fname,RandomReWrFile **result)=0;
...
// Unlike WritableFile, this file type allows users to specify offset where data shall be written
class LEVELDB_EXPORT RandomReWrFile
{
public:
  RandomReWrFile()=default;

  RandomReWrFile(const RandomReWrFile&)=delete;
  RandomReWrFile& operator=(const RandomReWrFile&)=delete;

  virtual ~RandomReWrFile()=default;
  virtual Status Append(const Slice& data)=0;
  virtual Status Lseek(uint64_t offset)=0;
  
  // TODO bvehavior might be different from Read of Random access file
  virtual Status Read(uint64_t offset, size_t n, Slice* result,
                      char* scratch) const = 0;
  virtual Status Close()=0;
  virtual Status Sync()=0;
...
class LEVELDB_EXPORT EnvWrapper : public Env 
{
 public:
  ...
  explicit EnvWrapper(Env* t) : env_target_(t) {}
  virtual ~EnvWrapper();

  // Return the target to which this Env forwards all calls.
  Env* target() const { return env_target_; }

  // The following text is boilerplate that forwards all methods to target().
  Status NewSequentialFile(const std::string& f, SequentialFile** r) override 
  {
    return env_target_->NewSequentialFile(f, r);
  }
  Status NewRandomAccessFile(const std::string& f,
                             RandomAccessFile** r) override {
    return env_target_->NewRandomAccessFile(f, r);
  }
  Status NewWritableFile(const std::string& f, WritableFile** r) override {
    return env_target_->NewWritableFile(f, r);
  }

  Status OpenFileAsRandomReWrFile(const std::string& f,RandomReWrFile **r) override
  {
    env_target_->OpenFileAsRandomReWrFile(f,r);
  }

  Status NewAppendableFile(const std::string& f, WritableFile** r) override {
    return env_target_->NewAppendableFile(f, r);
  }
  bool FileExists(const std::string& f) override {
    return env_target_->FileExists(f);
  }
  Status GetChildren(const std::string& dir,
                     std::vector<std::string>* r) override {
    return env_target_->GetChildren(dir, r);
  }
  Status RemoveFile(const std::string& f) override {
    return env_target_->RemoveFile(f);
  }
  Status CreateDir(const std::string& d) override {
    return env_target_->CreateDir(d);
  }
  Status RemoveDir(const std::string& d) override {
    return env_target_->RemoveDir(d);
  }
  Status GetFileSize(const std::string& f, uint64_t* s) override {
    return env_target_->GetFileSize(f, s);
  }
  Status RenameFile(const std::string& s, const std::string& t) override {
    return env_target_->RenameFile(s, t);
  }
  Status LockFile(const std::string& f, FileLock** l) override {
    return env_target_->LockFile(f, l);
  }
  Status UnlockFile(FileLock* l) override { return env_target_->UnlockFile(l); }
  void Schedule(void (*f)(void*), void* a) override {
    return env_target_->Schedule(f, a);
  }
